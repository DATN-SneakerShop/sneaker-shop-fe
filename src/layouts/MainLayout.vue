<script setup>
import { ref, watch, onMounted, computed } from 'vue'; // Thêm computed
...
const username = ref('');
const roles = ref([]); // Đổi từ userRole thành roles (mảng)
...
const syncUserInfo = () => {
    ...
    try {
        // Lấy userRoles (mảng) thay vì userRole (chuỗi)
        ...
        <a-layout>
            <a-layout-header
                ...
                <div style="display: flex; align-items: center; gap: 12px">
                    ...
                    <a-tag color="orange" v-for="role in roles" :key="role">{{ role }}</a-tag>
                </div>
            </a-layout-header>
